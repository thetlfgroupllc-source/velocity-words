...
<body>
    ...
    ">
        ...
        <div style="color: #999; font-size: 1rem;">Checking access...</div>
    </div>

    <script type="module">
        // Firebase Authentication & Subscription Check
        ...
        const firebaseConfig = {
            apiKey: "AIzaSyADOpIXsX18BNAB22UMcZ-77hnQmhLFKrY",
            authDomain: "velocity-words-speed-rea-d8c72.firebaseapp.com",
            projectId: "velocity-words-speed-rea-d8c72",
            storageBucket: "velocity-words-speed-rea-d8c72.firebasestorage.app",
            messagingSenderId: "853548916724",
            appId: "1:853548916724:web:a250be38eef21d936123a1"
        };
        ...
        onAuthStateChanged(auth, async (user) => {
            ...
            try {
                ...
                if (!userDoc.exists()) {
                    // User in Auth but not Firestore - redirect to signup/pricing
                ...
                // Allow access for: active, past_due (grace period)
                ...
                if (allowedStatuses.includes(subscriptionStatus)) {
                    ...
                    alert('Your subscription is not active. Redirecting to pricing page.');
                    window.location.href = 'index.html#pricing';
                }
...
function VelocityWordsReader() {
    ...
    useEffect(() => {
        ...
        const subscriptionStatus = localStorage.getItem('velocity-subscription');
        ...
        const handleKeyDown = (e) => {
            ...
            if (e.code === customShortcuts.playPause) {
                ...
                if (words.length > 0 && words[currentIndex]) {
                    ...
                    setToast({ message: '✓ Highlighted', type: 'info' });
    ...
    const loadText = () => {
        if (inputText.trim()) {
            // Check if user is using sample text or has subscription
            const isSampleText = inputText.trim() === sampleText.trim();
            
            if (!isSampleText && !isSubscribed) {
                setShowPaywall(true);
                return;
            }
    ...
    const handlePDFUpload = async (file) => {
        ...
        try {
            ...
            setIsLoading(false);
            showToast(`✓ Uploaded PDF: ${file.name} (${Math.min(pdf.numPages, maxPages)} pages)`);
        } catch (error) {
    ...
    const handleEPUBUpload = async (file) => {
        ...
        try {
            ...
            setIsLoading(false);
            showToast(`✓ Uploaded EPUB: ${file.name}`);
        } catch (error) {
    ...
    const endSession = (finalIndex, completed = false) => {
        ...
        if (lastReadDate !== today) {
            ...
            if (newStreak > 1) {
                setTimeout(() => showToast(`🔥 ${newStreak} day streak!`, 'success'), 1000);
            }
        }

        // Show completion notification ONLY if document was completed
        if (completed && sessionWordsRead >= 10) {
            showToast(`🎉 Completed! ${sessionWordsRead} words at ${actualWPM} WPM`, 'success');
        }
    ...
    const completeFocusExercise = () => {
        ...
        showToast('✓ Focus exercise complete!', 'success');
    };
    ...
    const commands = [
        { id: 'play', name: 'Play/Pause', action: () => words.length > 0 && setIsPlaying(p => !p), icon: '▶️' },
        { id: 'reset', name: 'Reset to Start', action: handleReset, icon: '↺' },
        { id: 'skipback', name: 'Skip Backward', action: skipBackward, icon: '⏪' },
        { id: 'skipforward', name: 'Skip Forward', action: skipForward, icon: '⏩' },
        { id: 'highlight', name: 'Highlight Word', action: addHighlight, icon: '⭐' },
        { id: 'note', name: 'Add Note', action: () => { setShowNoteInput(true); setIsPlaying(false); }, icon: '📝' },
        { id: 'settings', name: 'Open Settings', action: () => setShowSettings(true), icon: '⚙️' },
        { id: 'stats', name: 'View Statistics', action: () => setShowStats(true), icon: '📊' },
        { id: 'docs', name: 'My Documents', action: () => setShowDocuments(p => !p), icon: '📚' },
        { id: 'export', name: 'Export Settings', action: exportSettings, icon: '📥' },
        { id: 'focus', name: 'Focus Exercise', action: startFocusExercise, icon: '🎯' }
    ];
        ...
        <div className="flex flex-col min-h-screen p-3 sm:p-5 transition-all duration-300" style={{ background: currentTheme.bg, color: currentTheme.text }}>
            ...
            <div className="rounded-2xl p-4 sm:p-5 mb-4 sm:mb-5 transition-all backdrop-blur-md" 
                ...
                <div className="flex items-center justify-between mb-3">
                    ...
                    >
                        📚 {showDocuments ? 'Hide' : 'My Docs'} ({savedDocuments.length})
                    </button>
                </div>
                ...
                <textarea
                    ...
                    onChange={(e) => {
                        ...
                        // Allow editing if it's still the sample text or user is subscribed
                        if (isSubscribed || newText.trim() === '' || newText.trim() === sampleText.trim()) {
                            setInputText(newText);
                        } else {
                            // User is trying to enter custom text without subscription
                            setShowPaywall(true);
                        }
                    }}
                    placeholder="Enter text to read... (Subscribe to use custom text)"
                    onFocus={(e) => e.target.style.borderColor = currentTheme.focusColor}
                    onBlur={(e) => e.target.style.borderColor = currentTheme.border}
                />
                ...
                <div className="grid grid-cols-3 gap-2 mt-2">
                    ...
                    <button 
                        ...
                        disabled={!inputText.trim()}
                    >
                        ▶ Load ({inputText.trim().split(/\s+/).filter(w => w).length})
                    </button>
                </div>
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs" style={{ color: colorTheme === 'light' ? '#666' : '#888' }}>
                                            {doc.wordCount} words
                                            {doc.lastPosition > 0 && ` • ${Math.round((doc.lastPosition / doc.wordCount) * 100)}% complete`}
                                        </p>
                ...
                {words.length === 0 ? (
                    <div className="text-center px-4 py-12">
                        ...
                        <div className="sm:hidden rounded-lg p-3 mb-4 text-xs text-left max-w-xs mx-auto" style={{ backgroundColor: currentTheme.focusColor, border: `1px solid ${currentTheme.focusColor}` }}>
                            <p className="font-bold mb-2" style={{ color: '#000' }}>📱 Mobile Tips:</p>
                            <ul className="space-y-1 text-left" style={{ color: '#000' }}>
                                <li>• Load text, then tap <strong>▶ Play</strong></li>
                                <li>• Swipe left/right to skip words</li>
                                <li>• Tap <strong>⚙️ Settings</strong> to customize</li>
                            </ul>
                        </div>
                                ...
                                {readingHistory.slice(0, 10).map(session => (
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-4 text-xs" style={{ color: colorTheme === 'light' ? '#666' : '#888' }}>
                                            <span>📖 {session.wordsRead} words</span>
                ...
                <div className="mb-5 sm:mb-6">
                    ...
                    <div className="flex gap-2 mt-2 flex-wrap">
                        {[
                            ...
                            { label: 'Fast', speed: 650 },
                            { label: 'Very Fast', speed: 1000 }
                        ].map(preset => (
                            <button 
                                key={preset.speed}
            ...
            {words.length > 0 && (
                <div className="rounded-2xl p-4 sm:p-5 mt-4 transition-colors" style={{ background: currentTheme.panel, border: `1px solid ${currentTheme.border}`, boxShadow: currentTheme.shadow, backdropFilter: "blur(10px)" }}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
                        ...
                        >
                            🔄 {reReadStart === null ? 'Mark' : reReadEnd === null ? 'End' : 'Ready'}
                        </button>
                    ...
                    {(highlights.length > 0 || notes.length > 0) && (
                        <div className="mt-3 space-y-2 max-h-[200px] overflow-y-auto">
                            {highlights.map(highlight => (
                                ...
                                >
                                    <span className="text-sm">⭐ <strong>{highlight.word}</strong></span>
                                    <button
                            ...
                            {notes.map(note => (
                                ...
                                >
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            <p className="text-sm font-semibold">📝 {note.word}</p>
                    ...
                    >
                        ...
                        {onboardingStep === 2 && (
                            <>
                                ...
                                <div className="space-y-3 mb-6 text-sm">
                                    <p><strong>📱 Mobile:</strong> Swipe left/right to skip words</p>
                                    <p><strong>📄 Files:</strong> Upload PDF, DOCX, EPUB, TXT (under 50MB)</p>
                                    <p><strong>☁️ Cloud:</strong> Download from iCloud/Drive first, then upload</p>
                                    <p><strong>⌨️ Keyboard:</strong> Space = play/pause, arrows = skip</p>
                                    <p><strong>⭐ Highlight:</strong> Press H to mark important words</p>
                                    <p><strong>📝 Notes:</strong> Press N to add notes</p>
                                    <p><strong>💾 Save:</strong> Documents auto-save your position</p>
                                    <p><strong>📊 Stats:</strong> Track your reading progress</p>
                                </div>
                                <button
                                    className="w-full py-3 rounded-lg font-semibold transition-all touch-manipulation hover:opacity-90"
                                    style={{ backgroundColor: currentTheme.focusColor, color: '#fff' }}
            ...
            {showPaywall && (
                ...
                >
                    <div 
                        className="max-w-md w-full rounded-2xl p-6 sm:p-8"
                        style={{ background: currentTheme.panel, border: `2px solid ${currentTheme.focusColor}`, boxShadow: `0 0 40px ${currentTheme.focusColor}80`, backdropFilter: "blur(10px)" }}
                        onClick={(e) => e.stopPropagation()}
                    >
                        <div className="text-center mb-6">
                            <div className="text-5xl mb-4">🚀</div>
                            <h2 className="text-2xl font-bold mb-3" style={{ color: currentTheme.focusColor }}>
                                Subscribe to Unlock
                            </h2>
                            <p className="text-sm mb-4" style={{ color: colorTheme === 'light' ? '#666' : '#888' }}>
                                You've tried the sample! Subscribe now to read your own documents and text at superhuman speeds.
                            </p>
                        </div>

                        <div className="space-y-3 mb-6">
                            <div className="flex items-start gap-3 text-sm">
                                <span style={{ color: currentTheme.focusColor }}>✓</span>
                                <span>Upload unlimited PDF, DOCX, EPUB files</span>
                            </div>
                            <div className="flex items-start gap-3 text-sm">
                                <span style={{ color: currentTheme.focusColor }}>✓</span>
                                <span>Paste and read any custom text</span>
                            </div>
                            <div className="flex items-start gap-3 text-sm">
                                <span style={{ color: currentTheme.focusColor }}>✓</span>
                                <span>Save documents and track progress</span>
                            </div>
                            <div className="flex items-start gap-3 text-sm">
                                <span style={{ color: currentTheme.focusColor }}>✓</span>
                                <span>Read 3-5x faster with speed reading technology</span>
                            </div>
                        </div>
                        
                        <div className="space-y-3">
                            <button
                                className="w-full py-4 rounded-lg font-bold text-lg transition-all touch-manipulation hover:opacity-90"
                                style={{ backgroundColor: currentTheme.focusColor, color: '#fff', boxShadow: `0 4px 20px ${currentTheme.focusColor}60` }}
                                onClick={() => {
                                    window.location.href = '/index.html#pricing';
                                }}
                            >
                            ...
                            <button
                                ...
                                style={{ backgroundColor: 'transparent', color: currentTheme.text, border: `1px solid ${currentTheme.border}` }}
                                onClick={() => setShowPaywall(false)}
                            >
